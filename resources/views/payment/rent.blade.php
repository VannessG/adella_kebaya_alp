...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <h1 class="fw-bold text mb-4">Pembayaran Sewa</h1>

            <!-- Ringkasan Sewa -->
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-light">
                    <h5 class="card-title mb-0 fw-semibold">Detail Penyewaan</h5>
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <p><strong>No. Sewa:</strong> {{ $rent->rent_number }}</p>
                            <p><strong>Periode:</strong> 
                                {{ $rent->start_date->format('d M Y') }} - {{ $rent->end_date->format('d M Y') }}
                            </p>
                            <p><strong>Lama Sewa:</strong> {{ $rent->calculateRentalDays() }} hari</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Cabang:</strong> {{ $rent->branch->name }}</p>
                    ...
                    <!-- Daftar Produk -->
                    <h6 class="fw-bold mb-3">Produk Disewa:</h6>
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted">
                                        Rp {{ number_format($product->pivot->price_at_rent, 0, ',', '.') }}/hari × 
                                        {{ $product->pivot->quantity }} item × {{ $rent->calculateRentalDays() }} hari
                                    </small>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6 text-end">
                            <strong>Rp {{ number_format($product->pivot->subtotal, 0, ',', '.') }}</strong>
                        </div>
                    </div>
                    @endforeach
                    ...
                    <div class="border-top pt-3 mt-3">
                        <div class="row">
                            <div class="col-md-6">
                                <p class="mb-1">Subtotal Sewa ({{ $rent->calculateRentalDays() }} hari)</p>
                            ...
                            <div class="col-md-6 text-end">
                                <p class="mb-1">Rp {{ number_format($rent->total_amount - $rent->shipping_cost, 0, ',', '.') }}</p>
                                @if($rent->shipping_cost > 0)
                                <p class="mb-1">Rp {{ number_format($rent->shipping_cost, 0, ',', '.') }}</p>
                                @endif
                                <p class="mb-0 fw-bold fs-5 text">Rp {{ number_format($rent->total_amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <form action="{{ route('payment.rent.process', $rent->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('pesanan.show', $rent->rent_number) }}" class="btn btn-outline-secondary">
