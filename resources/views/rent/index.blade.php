...
<div class="container pb-4">
    <h1 class="display-5 fw-normal text-uppercase text-black mb-2 text-center text-md-start font-serif h3">Riwayat Penyewaan</h1>
...
<div class="container pb-5 px-0 px-md-3">
    @if(count($rents) > 0)
        <div class="rent-list-container shadow-sm">
            <div class="rent-list-header">
                <div>Info Sewa</div>
                <div>Total</div>
            </div>
            ...
            @foreach($rents as $rent)
                <div class="rent-list-item">
                    <div class="col-thumb">
                        @if($rent->products->isNotEmpty())
                            <img src="{{ $rent->products->first()->image_url }}" alt="Product" class="rent-img">
                        @else
                            <div class="rent-img bg-light d-flex align-items-center justify-content-center text-muted">
                    ...
                    <div class="col-info">
                        <span class="rent-number">{{ $rent->rent_number }}</span>
                        <span class="rent-date">{{ $rent->start_date->format('d/m/y') }} - {{ $rent->end_date->format('d/m/y') }}</span>
                        <div class="btn-action-group">
                            <a href="{{ route('rent.show', $rent->rent_number) }}" class="btn-custom-outline">Detail</a>

                            @if($rent->status == 'completed' || $rent->status == 'returned')
                                @foreach($rent->products as $product)
                                    @if(!$rent->userProductReview($product->id))
                                        <button class="btn-custom-black" data-bs-toggle="modal" data-bs-target="#revRent{{ $rent->id }}{{ $product->id }}">Ulas</button>
                    ...
                    <div class="col-total">
                        <span class="rent-price">Rp {{ number_format($rent->total_amount, 0, ',', '.') }}</span>
                        <span class="badge-status-custom" style="background-color: {{ $rent->status_style['bg'] }}; color: {{ $rent->status_style['color'] }}; border: {{ $rent->status_style['border'] }};">
                            {{ $rent->status_style['label'] }}
                        </span>
                    </div>
                </div>
            @endforeach
    ...
    @else
        <div class="text-center py-5 border" style="border-style: dashed !important; border-color: #E0E0E0 !important;">
            ...
            <h4 class="fw-normal text-uppercase text-muted mb-4" style="letter-spacing: 0.1em; font-size: 0.8rem;">Belum ada riwayat penyewaan.</h4>
            <a href="{{ url('/katalog') }}" class="btn-custom-outline" style="font-size: 0.8rem; letter-spacing: 0.1em;">Sewa Sekarang</a>
        </div>
    @endif
</div>
@endsection