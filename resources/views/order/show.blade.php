...
<div class="container pb-4">
    <div class="row align-items-end">
        <div class="col-12 col-md-8 text-center text-md-start mb-3 mb-md-0">
            <h1 class="display-5 fw-normal text-uppercase text-black mb-2 font-serif h3">Detail Pembelian</h1>
            <p class="text-muted small text-uppercase mb-0" style="letter-spacing: 0.1em;">No. Pesanan: <span class="text-black fw-bold text-break">{{ $order->order_number }}</span></p>
        </div>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            ...
            <div class="row g-4 mb-5 mx-0 mx-md-0">
                <div class="col-12 col-md-6">
                    <div class="card h-100 border rounded-0 bg-white p-4">
                        ...
                        <div class="d-flex justify-content-between mb-0 small text-uppercase">
                            ...
                            <span class="text-black text-end text-break">{{ $order->customer_address }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6">
                    <div class="card h-100 border rounded-0 bg-white p-4">
                        <h5 class="fw-normal text-uppercase text-black mb-4 border-bottom border-black pb-2 font-serif h6">Informasi Pesanan</h5>
                        <div class="d-flex justify-content-between mb-2 small text-uppercase">
                            <span class="text-muted">Tanggal Pesan</span>
                            <span class="text-black text-end">{{ $order->order_date }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2 small text-uppercase">
                            <span class="text-muted">No. Pesanan</span>
                            <span class="text-black fw-bold text-end text-break">{{ $order->order_number }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border rounded-0 shadow-sm mb-5 mx-0 mx-md-0">
                <div class="card-header bg-white border-bottom p-4"><h5 class="card-title mb-0 fw-normal text-uppercase text-black font-serif h6">Detail Produk</h5></div>
                    ...
                    <table class="table align-middle mb-0">
                        <thead class="bg-light border-bottom border-black">
                            <tr>
                                ...
                                <th class="text-end py-3 text-uppercase small text-muted font-weight-bold">Harga</th>
                            ...
                            <tr>
                                <td class="ps-4 py-3">
                                    <div class="d-flex align-items-center">
                                        <img src="{{ $product->image_url }}" style="width: 40px; height: 40px; object-fit: cover;" class="me-3 border">
                                ...
                                <td class="text-end pe-4 py-3 fw-bold text-black small">Rp {{ number_format($product->pivot->price * $product->pivot->quantity, 0, ',', '.') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="card-body p-4 d-md-none">
                    ...
                    <div class="product-list-item">
                        ...
                        <div class="product-details">
                            <span class="product-name text-black">{{ $product->name }}</span>
                            <div class="product-meta">{{ $product->pivot->quantity }} x Rp {{ number_format($product->pivot->price, 0, ',', '.') }}</div>
                        </div>

                        <div class="product-subtotal text-black">Rp {{ number_format($product->pivot->price * $product->pivot->quantity, 0, ',', '.') }}</div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="card-footer bg-light border-top border-black p-4">
                    <div class="d-flex justify-content-between mb-2 small text-uppercase">
                        ...
                        <span class="fw-bold text-black">Rp {{ number_format($order->subtotal, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between border-top border-secondary pt-3 mt-2">
                        ...
                        <span class="fw-bold h5 mb-0 text-black font-serif">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row align-items-center g-4">
                        <div class="col-12 col-md-7">
                            ...
                            @if($order->payment->status === 'pending')
                                @if($order->payment->paymentMethod->type !== 'transfer')
                                    <div class="alert rounded-0 bg-light border border-dark text-black small mb-3 p-3">
                                        <i class="bi bi-info-circle me-2"></i> Silakan selesaikan pembayaran.
                                    </div>
                                    <a href="{{ route('payment.pay', $order->payment->payment_number) }}" class="btn btn-primary-custom rounded-0 w-100 py-3 fw-bold text-uppercase">Bayar Sekarang</a>
                                @else
                                    <form action="{{ route('payment.order.process', $order->id) }}" method="POST" enctype="multipart/form-data" class="mt-3 p-3 bg-light border">
                                        @csrf
                        ...
                        <div class="col-12 col-md-5 text-center border-start-md ps-md-4">
                            ...
                            <img src="{{ asset('storage/' . $order->payment->proof_image) }}" class="img-fluid border p-1 bg-white shadow-sm" style="max-height: 200px;">
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 pt-4 border-top border-black mx-0 mx-md-0">
                <a href="{{ url('/pesanan') }}" class="btn btn-link text-decoration-none text-muted text-uppercase small w-100 w-md-auto text-center text-md-start mb-2 mb-md-0">
                    <i class="bi bi-arrow-left me-2"></i> Kembali
                </a>
                <div class="d-flex flex-column flex-md-row gap-2 w-100 w-md-auto">
                    @if($order->canBeCancelled())
                        <form action="{{ route('pesanan.cancel', $order) }}" method="POST" class="w-100 w-md-auto">
                            @csrf
                            <button type="submit" class="btn btn-outline-danger rounded-0 px-4 py-2 fw-bold text-uppercase w-100" onclick="return confirm('Batalkan pesanan ini?')">
                                Batalkan
                            </button>
                        </form>
                    @endif
