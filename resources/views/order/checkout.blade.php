...
<div class="container pb-5">
    <form action="{{ route('checkout') }}" method="POST" enctype="multipart/form-data" id="checkoutForm">
        ...
        {{-- Jika checkout langsung (bukan dari keranjang), kirim data produk --}}
        ...
        <div class="row g-5">
            ...
            <div class="col-lg-8">
                ...
                {{-- 2. METODE PENGIRIMAN (STRUKTUR DIPERBAIKI SESUAI RENT CREATE) --}}
                <div class="card border rounded-0 mb-5 bg-white" style="border-color: var(--border-color);">
                    <div class="card-header bg-white border-bottom p-4" style="border-color: var(--border-color) !important;">
                    ...
                    <div class="card-body p-4">
                        ...
                        {{-- Form Alamat & Kurir (Hidden by default) --}}
                    ...
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <div class="flex-grow-1 overflow-hidden">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <h6 class="text-uppercase small fw-bold text-black mb-0 text-break pe-2" style="letter-spacing: 0.05em; line-height: 1.4;">
                                    {{ $item['name'] }}
                                </h6>
                            </div>
                            <div class="d-flex justify-content-between align-items-center">
                                <p class="mb-0" style="font-size: 0.75rem;">Rp {{ number_format($item['subtotal'], 0, ',', '.') }}</p>
                                <p class="mb-0" style="font-size: 0.75rem;">Qty: {{ $item['quantity'] }}</p>
                            </div>
                        </div>
                    </div>
                    @endforeach
...
$(document).ready(function() {
    ...
    // 1. Toggle Ekspedisi / Pickup
    $('.delivery-type').change(function() {
        if(this.value === 'delivery') {
            $('#delivery-section').slideDown();
            calculateShipping(); // Coba hitung jika data sudah ada
        } else {
    ...
    $('#city_id').on('change', function() {
        ...
        if(id) {
            ...
            // Tidak auto calculate saat pilih kota, tunggu kecamatan
        }
    });
    ...
    function calculateShipping() {
        ...
        $.ajax({
            ...
            method: "GET",
            // Asumsi berat total 1kg untuk keranjang, bisa disesuaikan dengan total berat item
            ...
            success: function(response) {
                ...
                if(response.costs && response.costs.length > 0) {
                    ...
                    // Auto select REG atau layanan pertama
                    let autoSelect = false;
                    $("#courier_service option").each(function() {
                        // Prioritaskan REG atau layanan standar
    ...
    // 5. Toggle Upload Bukti Transfer
