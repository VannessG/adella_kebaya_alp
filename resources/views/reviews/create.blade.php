...
<div class="container pb-4">
    <div class="row align-items-end">
        <div class="col-12 col-md-8 text-center text-md-start mb-3 mb-md-0">
            <h1 class="display-5 fw-normal text-uppercase text-black mb-2 font-serif h3">Beri Ulasan</h1>
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-8">
            <div class="card border rounded-0 bg-white p-4 p-md-5 shadow-sm" style="border-color: var(--border-color);">
                ...
                <div class="bg-light p-4 border border-secondary-subtle mb-5">
                    <h5 class="fw-normal text-uppercase text-black mb-3 pb-2 border-bottom border-secondary-subtle" style="font-family: 'Marcellus', serif; letter-spacing: 0.1em; font-size: 1rem;">Detail Transaksi</h5>
                ...
                <form action="{{ route('reviews.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        @error('product_id') <div class="invalid-feedback small">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-2 align-items-center star-rating-container">
                            @for($i = 1; $i <= 5; $i++)
                                <input type="radio" class="rating-radio" name="rating" id="star{{ $i }}" value="{{ $i }}" {{ old('rating') == $i ? 'checked' : '' }} required>
                                <label class="star-label" for="star{{ $i }}">
                                    <i class="bi bi-star star-icon-empty"></i>
                                    <i class="bi bi-star-fill star-icon-fill"></i>
                                </label>
                            @endfor
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control rounded-0 bg-subtle border-0 p-3 @error('comment') is-invalid @enderror" id="comment" name="comment" rows="4" placeholder="Ceritakan pengalaman Anda..." required>{{ old('comment') }}</textarea>
                        @error('comment') <div class="invalid-feedback small">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="d-flex flex-column flex-md-row gap-3">
                        <button type="submit" class="btn btn-primary-custom rounded-0 py-3 px-5 text-uppercase fw-bold w-100 w-md-auto flex-grow-1" style="font-size: 0.8rem; letter-spacing: 0.1em;">Kirim Ulasan</button>
                        <a href="{{ route('home') }}" class="btn btn-outline-custom rounded-0 py-3 px-5 text-uppercase fw-bold w-100 w-md-auto flex-grow-1 text-center" style="font-size: 0.8rem; letter-spacing: 0.1em;">Nanti Saja</a>
                    </div>
                </form>
            </div>
        </div>
